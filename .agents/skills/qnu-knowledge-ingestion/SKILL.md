...
description: >-
  Use this skill when implementing document parsers, tuning chunking algorithms (Clause-based vs Semantic),
  cleaning OCR/text artifacts, or optimizing the document ingestion pipeline on QNU AI Platform.
---

# Hướng Dẫn Bóc Tách Tài Liệu & Quản Trị Tri Thức QNU AI Platform

Tài liệu hướng dẫn chuyên sâu cho module **Knowledge & Document Intelligence** ([app/modules/knowledge/](file:///d:/DuAnPhanMem/DeTaiAI/qnu-ai-platform/backend/app/modules/knowledge)).
...
2. **Lưu trữ tệp gốc**:
   - Gọi `StorageDriver` (`LocalStorageDriver` hoặc `S3StorageDriver`) để lưu trữ an toàn.
...
1. Tạo class mới kế thừa `BaseDocumentParser` trong [parsers/](file:///d:/DuAnPhanMem/DeTaiAI/qnu-ai-platform/backend/app/modules/knowledge/parsers):
   ```python
