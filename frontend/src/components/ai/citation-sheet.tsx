import { BookOpen, ExternalLink, FileCheck2, Layers, Quote, ShieldCheck } from "lucide-react";
...
export const CitationSheet: React.FC<CitationSheetProps> = ({ citation, isOpen, onOpenChange }) => {
  if (!citation) return null;

  const scorePct = citation.score !== undefined ? Math.round(citation.score * 100) : undefined;
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5 text-sm">
          ...
          <div className="grid grid-cols-2 gap-3 p-3.5 rounded-control bg-muted/50 border border-border/80 text-xs">
            <div className="space-y-1">
              ...
              <p className="font-semibold text-foreground truncate" title={citation.document_name}>
            ...
            <div className="space-y-1">
              ...
              <p className="font-semibold text-foreground">
                {[citation.article, citation.clause].filter(Boolean).join(", ") ||
                  "Điều khoản chung"}
              </p>
            </div>
            ...
            {citation.page !== undefined && (
              <div className="space-y-1">
                ...
                <p className="font-semibold text-foreground">Trang {citation.page}</p>
              </div>
            )}

            <div className="space-y-1">
              <span className="text-muted-foreground font-medium">Thuật toán trích xuất</span>
          ...
          <div className="p-3 rounded-control bg-muted/80 border border-border/80 text-[11px] text-muted-foreground leading-relaxed">
            <span className="font-semibold text-foreground">Cam kết Zero-Hallucination:</span> Mọi
            câu trả lời của Trợ lý AI QNU đều được kiểm tra tính xác thực qua bộ lọc Guardrails
            trước khi chuyển tiếp tới người dùng. Nếu thông tin không có trong tài liệu, hệ thống tự
            động điều hướng tới Phòng ban phụ trách.
          </div>
        </div>
