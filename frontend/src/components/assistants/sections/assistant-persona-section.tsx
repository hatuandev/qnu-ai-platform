...
import { type AssistantEditForm, CATEGORY_OPTIONS } from "@/components/assistants/types";
...
export function AssistantPersonaSection({
  ...
  const handleUpdateQuestion = (id: string, text: string) => {
    onChange({
      ...form,
      sample_questions: form.sample_questions.map((q) => (q.id === id ? { ...q, text } : q)),
    });
  };
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-3">
          <div>
            ...
            <CardDescription className="text-xs">
              Các câu hỏi mẫu hiển thị trên khung chat giúp sinh viên / giảng viên tra cứu nhanh.
            </CardDescription>
          </div>
          <Button
            type="button"
