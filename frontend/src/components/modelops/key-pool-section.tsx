...
export interface KeyPoolSectionProps {
  ...
  rotationResult: { success: boolean; rotated: boolean; message: string } | null;
  ...
  onToggleKeyActive: (providerId: string, keyId: string, isActive: boolean) => void;
}
      ...
      <div className="lg:col-span-2 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-border/70">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Hỗ trợ nạp nhiều API key, tự động xoay vòng khi chạm Rate Limit 429 hoặc cạn Token
                Quota.
              </p>
            </div>
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewKeyPriority(Number.parseInt(e.target.value, 10) || 1)}
                    className="h-8 text-xs"
                  />
              ...
              <div className="flex justify-end gap-2 pt-1 border-t border-border/60">
                ...
                <Button
                  type="submit"
                  size="sm"
                  disabled={!newKeyName.trim() || !newKeySecret.trim() || isAddingKey}
                  className="h-8 text-xs"
                >
            ...
            {loadingKeys ? (
              ...
              <div className="p-8 text-center text-xs text-muted-foreground border border-dashed rounded-md space-y-2">
                ...
                <p className="text-muted-foreground">
                  Bấm nút "Thêm Khóa Mới" ở trên để thiết lập khóa đầu tiên cho Provider này.
                </p>
              </div>
            ) : (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <Switch
                            ...
                            onCheckedChange={(checked) =>
                              onToggleKeyActive(selectedProvider.id, keyItem.id, checked)
                            }
                      ...
                      <div className="flex items-center justify-between border-t border-border/50 pt-2 text-xs">
                        <Button
                          ...
                          onClick={() => onTestSingleKey(selectedProvider.id, keyItem.id)}
        ...
        <Card className="p-5 space-y-3 bg-muted/10 border-border/70">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            Khi thực hiện suy luận LLM, Gateway sẽ tự động chọn khóa khả dụng có độ ưu tiên cao nhất
            (#1). Nếu khóa đó gặp lỗi{" "}
            <code className="text-warning">HTTP 429 Too Many Requests</code> hoặc chạm hạn ngạch
            token, hệ thống lập tức chuyển sang khóa dự phòng kế tiếp và đưa khóa cũ vào trạng thái
            Cooldown trong 60 giây mà không làm gián đoạn trải nghiệm người dùng.
          </p>
        </Card>
      </div>
        ...
        <Card className="p-5 space-y-3 border-border">
          ...
          <div className="space-y-2 text-xs font-mono">
            <div className="flex justify-between py-1 border-b border-border/50">
              ...
              <span className="text-foreground truncate max-w-[150px]">{selectedProvider.id}</span>
            </div>
            ...
            <div className="flex justify-between py-1 border-b border-border/50">
              ...
              <span
                className={selectedProvider.is_active ? "text-success" : "text-muted-foreground"}
              >
