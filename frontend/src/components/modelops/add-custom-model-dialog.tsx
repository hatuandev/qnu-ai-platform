      ...
      <DialogContent className="sm:max-w-lg p-6 space-y-4 border-border shadow-xl">
        ...
        <DialogHeader className="space-y-1">
          ...
          <DialogDescription className="text-xs text-muted-foreground">
            Cấu hình ID mô hình tương thích với Provider {selectedProvider.name} và kiểm tra tính
            khả dụng trực tiếp trước khi kích hoạt.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-2 pt-1">
          ...
          <p className="text-[11px] text-muted-foreground font-mono">
            Gửi tới nhà cung cấp dưới dạng: {newCustomModelId.trim() || "model-id"}
          </p>
          ...
          {newModelTestResult && (
            ...
            >
              ...
              <div className="flex-1 font-medium">
                {newModelTestResult.status === "available" ? (
                  <span>
                    Khả dụng! Độ trễ phản hồi: <strong>{newModelTestResult.latency_ms}ms</strong> (
                    {newModelTestResult.message})
                  </span>
                ) : newModelTestResult.status === "rate_limited" ? (
                  <span>Chạm Rate Limit (429): {newModelTestResult.message}</span>
                ) : (
                  <span>Không khả dụng (404/Error): {newModelTestResult.message}</span>
                )}
              </div>
            </div>
          )}
