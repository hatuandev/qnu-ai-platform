import { AlertCircle, CheckCircle2, FileJson, Layers, RefreshCw, UploadCloud } from "lucide-react";
...
import type { ConflictStrategy, ProviderImportResponse } from "../../types/modelops";
...
export const ImportProvidersDialog: React.FC<ImportProvidersDialogProps> = ({
  ...
  const [importResult, setImportResult] = useState<ProviderImportResponse | null>(null);
  ...
  const parseAndSetJson = (content: string, fileName: string) => {
    try {
      ...
      if (json && typeof json === "object") {
        ...
        if (rawObj.export_type === "single_provider" && typeof rawObj.provider === "object") {
          ...
          items.push({
            ...
            modelsCount: Array.isArray(p.models) ? p.models.length : p.model_name ? 1 : 0,
            keysCount: Array.isArray(p.api_keys) ? p.api_keys.length : 0,
            accountId: typeof p.account_id === "string" ? p.account_id : undefined,
          });
        } else if (rawObj.export_type === "all_providers" && Array.isArray(rawObj.providers)) {
        ...
        } else if (typeof rawObj.name === "string" && (rawObj.provider_type || rawObj.type)) {
          items.push({
            name: String(rawObj.name),
            providerType: String(rawObj.provider_type || rawObj.type || "custom"),
            modelsCount: Array.isArray(rawObj.models)
              ? rawObj.models.length
              : rawObj.model_name
                ? 1
                : 0,
            keysCount: Array.isArray(rawObj.api_keys) ? rawObj.api_keys.length : 0,
            accountId: typeof rawObj.account_id === "string" ? rawObj.account_id : undefined,
          });
        }
      }
      ...
      setParseError(`Định dạng tệp '${fileName}' không phải là cú pháp JSON hợp lệ.`);
    }
  };
  ...
  const handleExecuteImport = async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "Đã xảy ra lỗi khi nạp cấu hình.";
      setParseError(msg);
    } finally {
      setIsImporting(false);
    }
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="text-xs text-muted-foreground">
            Tải lên tệp sao lưu JSON chứa cấu hình của 1 Provider hoặc toàn bộ các Provider trong hệ
            thống.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="flex flex-col items-center justify-center gap-2">
              ...
              <div>
                <p className="text-sm font-semibold text-foreground">
                  {selectedFile ? selectedFile.name : "Kéo thả tệp JSON vào đây hoặc click để chọn"}
                </p>
                <p className="text-xs text-muted-foreground mt-0.5">
                  Hỗ trợ tệp cấu hình đơn lẻ hoặc tệp sao lưu toàn bộ (tối đa 10MB)
                </p>
              </div>
            </div>
          </button>
            ...
            <div className="space-y-3 pt-1">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="outline" className="text-[11px] font-mono">
                  {previewItems.length === 1 ? "1 Provider" : "Tất Cả Providers"}
                </Badge>
              </div>
                ...
                <table className="w-full text-left text-xs border-collapse">
                  ...
                  <tbody className="divide-y divide-border/60">
                    {previewItems.map((item) => (
                      <tr key={`${item.providerType}-${item.name}`} className="hover:bg-muted/20">
                        ...
                        <td className="py-2 px-3 text-center font-mono">{item.modelsCount}</td>
                        <td className="py-2 px-3 text-center font-mono">{item.keysCount}</td>
              ...
              <div className="space-y-1.5 pt-2 border-t border-border/70">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  ...
                  >
                    ...
                    />
                    <div>
                      <strong className="block font-medium text-foreground">Ghi Đè</strong>
                  ...
                  >
                    ...
                    />
                    <div>
                      <strong className="block font-medium text-foreground">Bỏ Qua</strong>
                      <span className="text-[11px] text-muted-foreground leading-tight block">
                  ...
                  >
                    ...
                    />
                    <div>
                      <strong className="block font-medium text-foreground">Tạo Mới</strong>
                      <span className="text-[11px] text-muted-foreground leading-tight block">
