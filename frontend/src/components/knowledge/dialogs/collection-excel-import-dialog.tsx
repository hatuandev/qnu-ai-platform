      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4 py-2 text-xs">
          <p className="text-muted-foreground leading-relaxed">
            Chọn tệp bảng tính <code>.xlsx</code>, <code>.xls</code> hoặc <code>.csv</code> chứa các
            cột số liệu (Điểm chuẩn, Chỉ tiêu, Học phí, Tổ hợp xét tuyển). Hệ thống sẽ tự động bóc
            tách từng dòng thành các facts định lượng.
          </p>
              ...
              {selectedFile ? (
                <div>
                  ...
                  <p className="text-muted-foreground text-xs mt-0.5">
                    {(selectedFile.size / 1024).toFixed(1)} KB — Nhấp để chọn tệp khác
                  </p>
                </div>
              ) : (
                <div>
                  <p className="font-medium text-foreground">Kéo thả hoặc nhấp để chọn tệp</p>
                  <p className="text-muted-foreground text-xs mt-0.5">Hỗ trợ .xlsx, .xls, .csv</p>
                </div>
              )}
            </label>
          </div>
