...
import type { DocumentTypeCategory, DocumentTypeInput } from "@/services/document-types-api";
    ...
    <form id={formId} className="space-y-4" onSubmit={onSubmit}>
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-foreground" htmlFor={`${formId}-code`}>
            Mã chuẩn
          </label>
          <input
            id={`${formId}-code`}
          ...
          />
          <p className="text-[11px] text-muted-foreground">Ví dụ: quyet_dinh, thong_bao</p>
        </div>
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-foreground" htmlFor={`${formId}-name`}>
            Tên hiển thị
          </label>
          <input
            id={`${formId}-name`}
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="space-y-1.5 sm:col-span-2">
          <label className="text-xs font-medium text-foreground" htmlFor={`${formId}-category`}>
            Nhóm phân loại
          </label>
          <Select
            disabled={isSubmitting}
            value={value.category}
            onValueChange={(category) => updateField("category", category as DocumentTypeCategory)}
          >
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-foreground" htmlFor={`${formId}-priority`}>
            Độ ưu tiên
          </label>
          <input
            id={`${formId}-priority`}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-foreground" htmlFor={`${formId}-retention`}>
            Thời hạn lưu trữ
          </label>
          <input
            id={`${formId}-retention`}
            ...
            onChange={(event) => updateField("retention_period", event.target.value)}
          />
        </div>
      ...
      <div className="space-y-1.5">
        <label className="text-xs font-medium text-foreground" htmlFor={`${formId}-description`}>
          Mô tả nghiệp vụ
        </label>
        <textarea
