...
import { Check, Copy, FileSpreadsheet, Search, Sliders, Table as TableIcon, X } from "lucide-react";
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      ...
      if (currentRows.length > 0) {
        ...
        const padded = currentRows.map((r) => r.concat(Array(maxCols - r.length).fill("")));
        resultSheets.push({
...
export const ExcelSpreadsheetViewer: React.FC<ExcelSpreadsheetViewerProps> = ({
  ...
  const activeSheetIdx = activeSheetIndex !== undefined ? activeSheetIndex : internalSheetIdx;
  ...
  const [selectedCell, setSelectedCell] = useState<{ r: number; c: number; val: string } | null>(
    null
  );
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 py-2 bg-muted/40 border-b border-border gap-2 flex-wrap min-h-10">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-muted-foreground whitespace-nowrap text-[11px]">
            ({currentSheet.total_rows || currentSheet.rows.length} hàng × {maxColumns} cột)
          </span>
        </div>
      ...
      <div className="flex items-center gap-2 px-3 py-1 bg-muted/20 border-b border-border/80 text-[11px]">
        <div className="flex items-center gap-1 font-mono font-bold text-primary min-w-14">
          ...
          <span>
            {selectedCell ? `${getExcelColumnLetter(selectedCell.c)}${selectedCell.r + 1}` : "A1"}
          </span>
        </div>
        ...
        <div className="flex-1 font-mono truncate text-muted-foreground">
          {selectedCell ? (
            ...
            <span className="italic">Bấm vào bất kỳ ô nào để xem và sao chép</span>
          )}
        </div>
        {selectedCell && (
          <Button
              ...
              return (
                <tr
                  ...
                  className={`border-b border-border/60 hover:bg-muted/30 transition-colors ${
                    isMatchSearch ? "bg-amber-500/10" : rIdx % 2 === 1 ? "bg-muted/10" : ""
                  }`}
                >
                  {/* Row index label */}
                  ...
                  {Array.from({ length: maxColumns }).map((_, cIdx) => {
                    ...
                    const isCellMatch =
                      searchQuery && val.toLowerCase().includes(searchQuery.toLowerCase());
                      ...
                      >
                        <button
                          ...
                          className={`w-full h-full transition-all cursor-cell max-w-72 truncate block ${
                            isCompact ? "px-2 py-0.5 text-[11px]" : "px-3 py-1.5 text-xs"
      ...
      <div className="flex items-center justify-between px-3 py-1 bg-muted/60 border-t border-border gap-1 shrink-0 overflow-x-auto">
        ...
        <div className="text-[11px] text-muted-foreground whitespace-nowrap">
          Đang xem sheet: <strong className="text-foreground">{currentSheet.name}</strong>
        </div>
      </div>
    </div>
  );
