...
export const RunBenchmarkDialog: React.FC<RunBenchmarkDialogProps> = ({
  ...
  const [selectedAssistant, setSelectedAssistant] = useState<string>("admissions");
  const [selectedDataset, setSelectedDataset] = useState<string>("qnu_admissions_benchmark");
  const [sampleSize, setSampleSize] = useState<number | undefined>(5);
  const [evalMethod, setEvalMethod] = useState<"heuristic" | "llm_judge">("heuristic");
  ...
  useEffect(() => {
    if (selectedAssistant) {
      const matched = datasets.find(
        (d) => d.assistant_code === selectedAssistant || d.id.includes(selectedAssistant)
      );
      if (matched) {
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription className="text-xs text-muted-foreground">
            Thiết lập Trợ lý AI, bộ dữ liệu benchmark chính thức và phương pháp chấm điểm để đo
            lường 3 chỉ số chuẩn Ragas TM-08.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-2 text-xs">
          ...
          <div className="space-y-1.5">
            <label htmlFor="eval-assistant-select" className="font-semibold text-foreground">
            ...
            >
              {assistants.length === 0 ? (
                <>
                  <option value="admissions">Trợ lý Tuyển sinh (admissions)</option>
                  <option value="regulations">Trợ lý Quy chế học vụ (regulations)</option>
                  <option value="library">Trợ lý Thư viện (library)</option>
                  <option value="drafting">Trợ lý Soạn thảo văn bản (drafting)</option>
                  <option value="exam_matrix">Trợ lý Đề thi & Ma trận Bloom (exam_matrix)</option>
                </>
              ) : (
          ...
          <div className="space-y-1.5">
            <label htmlFor="eval-dataset-select" className="font-semibold text-foreground">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              ...
              >
                ...
                <p className="text-[11px] text-muted-foreground mt-1 leading-snug">
                  Đo lường n-gram & từ khóa tức thì (~1-2s). 0 tốn token LLM, phù hợp CI/CD và kiểm
                  tra thường xuyên.
                </p>
              </button>
              ...
              >
                ...
                <p className="text-[11px] text-muted-foreground mt-1 leading-snug">
                  Mô hình LLM thẩm định ngữ nghĩa sâu sắc theo rubric TM-08 ĐH Quy Nhơn và giải
                  thích lý do chấm điểm.
                </p>
              </button>
            </div>
          </div>
          ...
          <DialogFooter className="pt-2">
            ...
            >
              ...
              <span>{runMutation.isPending ? "Đang Chạy Kiểm Định..." : "Bắt Đầu Kiểm Định"}</span>
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
