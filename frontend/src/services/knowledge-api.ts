...
export const knowledgeApi = {
  async getCollections(): Promise<KnowledgeCollection[]> {
    ...
    if (!res.ok) {
      throw new Error(`Không tải được danh sách kho tri thức (HTTP ${res.status}).`);
    }
  ...
  async getCollection(collectionId: string): Promise<KnowledgeCollection> {
    ...
    throw new Error(`Không tìm thấy kho tri thức '${collectionId}' (HTTP ${res.status}).`);
  },
  ...
  async getDocuments(
    ...
    if (!res.ok) {
      throw new Error(`Không tải được danh sách tài liệu (HTTP ${res.status}).`);
    }
    const data = await res.json();
    if (!Array.isArray(data)) {
      return [];
    ...
    return data.map((item, idx) => {
      const d = item as Record<string, unknown>;
      const fn = (d.filename as string) || (d.file_name as string) || `document_${idx + 1}.pdf`;
      const fs =
