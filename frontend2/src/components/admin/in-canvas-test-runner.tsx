...
function getResponseText(response: WorkflowExecutionResponse): string {
  ...
  return typeof output === "string"
    ? output
    : "Chưa có nội dung phản hồi từ workflow.";
}
...
export const InCanvasTestRunner: React.FC<InCanvasTestRunnerProps> = ({
  ...
  const [inputMessage, setInputMessage] = useState<string>(
    QUICK_PROMPTS_BY_WORKFLOW[workflowId]?.[0] ||
      "Xin chào, cho tôi hỏi thông tin.",
  );
  ...
  const quickPrompts =
    QUICK_PROMPTS_BY_WORKFLOW[assistantCode] ||
    QUICK_PROMPTS_BY_WORKFLOW.admissions;
    ...
    try {
      ...
      for (const nodeId of executed) {
        currentStates[nodeId] = {
          status:
            response.status === "paused_for_approval" &&
            nodeId === response.paused_node_id
              ? "waiting"
      ...
      setApprovalId(
        typeof response.outputs.approval_id === "string"
          ? response.outputs.approval_id
          : null,
      );
    } catch (error: unknown) {
      const message =
        error instanceof Error ? error.message : "Lỗi thực thi luồng DAG.";
  ...
  const handleApproveCheckpoint = async () => {
    ...
    setExecutionError(null);
    try {
      const response = await workflowsApi.decideApproval(
        lastResponse.execution_id,
        approvalId,
        {
          approved: true,
      ...
      const completedStates = Object.fromEntries(
        response.executed_nodes.map((nodeId) => [
          nodeId,
      ...
      setExecutionError(
        error instanceof Error
          ? error.message
          : "Không thể phê duyệt checkpoint.",
      );
    } finally {
      setIsExecuting(false);
    }
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3.5 max-h-[calc(85vh-130px)]">
        ...
        {lastResponse?.status === "paused_for_approval" &&
          approvalId &&
          !isHumanApproved && (
          ...
          <div className="border-t border-border pt-3 space-y-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                ...
                <Badge
                  variant="outline"
                  className="text-[10px] font-mono ml-1 text-primary"
                >
