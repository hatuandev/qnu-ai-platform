...
function summarizeConfigSchema(
  configSchema: Record<string, unknown> | undefined,
): string {
  if (!configSchema || typeof configSchema !== "object")
    return "Mặc định chuẩn";
...
export const CATALOG_NODE_ITEMS: NodeCatalogItem[] = [
  ...
  },
  {
    ...
    defaultConfig: {
      rules: [
        {
          id: "greeting_rule",
          match: { intent: "xin chào|hello|chào" },
          to: "greeting_output",
        },
      ],
  ...
  },
  {
    ...
    defaultConfig: {
      ...
      message:
        "Thông tin này hiện chưa có trong văn bản chính thức của Trường Đại học Quy Nhơn.",
    },
    timeoutSeconds: 5,
    status: "active",
    version: "1.0.0",
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">
              Kéo thả hoặc thêm Node vào đồ thị
            </p>
          </div>
        </div>
        <Button
          ...
          >
            <div className="flex items-center justify-between">
              <span className="font-semibold text-xs text-foreground group-hover:text-primary transition-colors flex items-center gap-1.5">
                ...
                {item.category === "human" && (
                  <UserCheck className="size-3 text-orange-500" />
                )}
