...
function getEffectiveType(box: DocumentBoundingBox): string {
  ...
  if (
    ["table", "title", "header", "text", "list", "signature", "stamp"].includes(
      lbl,
    )
  ) {
    return lbl;
...
export const DocumentBoundingVisualizer: React.FC<
  DocumentBoundingVisualizerProps
> = ({
  currentPage,
  totalPages,
  onPageChange,
  ...
  const filteredBoxes = useMemo(() => {
    return boundingBoxes.filter((box) => {
      ...
      if (selectedFilter === "stamp")
        return ["stamp", "signature"].includes(effectiveType);
      return true;
    });
    ...
    <div className="flex flex-col h-full bg-muted/20 border-r border-border select-none">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 px-3 py-2 bg-card border-b border-border/70 text-xs shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          {onRescanLayout && (
            ...
            >
              <Sparkles
                className={`size-3.5 ${isRescanning ? "animate-spin text-primary" : ""}`}
              />
          ...
          <div className="flex items-center gap-1 bg-muted/60 rounded px-1 py-0.5">
            ...
            <span className="font-mono text-[11px] min-w-8 text-center">
              {zoomLevel}%
            </span>
            <Button
              variant="ghost"
        ...
        >
          ...
          {currentImageSrc && !imageError ? (
            ...
            <div className="p-12 text-center flex flex-col items-center justify-center gap-3 bg-card min-h-[640px] aspect-[1/1.414] w-full">
              ...
              <p className="text-xs text-muted-foreground max-w-sm leading-relaxed">
                Đang đối soát trực quan trang số {currentPage}. Vui lòng xem văn
                bản và bảng biểu bóc tách ở cột Markdown bên phải.
              </p>
            </div>
          )}
          ...
          {showBoxes &&
            filteredBoxes.map((box) => {
              ...
              const effectiveType = getEffectiveType(box);
              const styleColor =
                REGION_COLORS[effectiveType] || REGION_COLORS.text;
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-card border-t border-border text-xs text-muted-foreground shrink-0">
        <span className="text-[11px]">
          Định dạng: Chuẩn BGDĐT / Nghị định 30
        </span>
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
