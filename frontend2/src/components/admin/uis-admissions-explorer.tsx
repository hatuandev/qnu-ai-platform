...
export const UisAdmissionsExplorer: React.FC<UisAdmissionsExplorerProps> = ({
  ...
  const [apiResponse, setApiResponse] = useState<ToolExecuteResponse | null>(
    null,
  );

  const activeMajor =
    UIS_MAJORS_DATABASE.find((m) => m.major_code === selectedMajorCode) ||
    UIS_MAJORS_DATABASE[0];
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 p-3 rounded-surface bg-muted/40 border border-border">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">
              Đồng bộ dữ liệu thời gian thực từ Cổng Thông Tin Đào Tạo Trường
              Đại học Quy Nhơn
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span>
              {isQuerying ? "Đang truy vấn UIS..." : "Truy Vấn Cổng UIS"}
            </span>
          </Button>
        </div>
      </div>
              ...
              <div className="grid grid-cols-3 gap-3 text-center">
                ...
                <div className="p-3 rounded-surface bg-muted/40 border border-border">
                  ...
                  <span className="text-[10px] text-success block">
                    +
                    {(
                      activeMajor.benchmark_2023 - activeMajor.benchmark_2022
                    ).toFixed(2)}{" "}
                    điểm
                  </span>
                </div>
                ...
                <div className="p-3 rounded-surface bg-primary/10 border border-primary/30">
                  ...
                  <span className="text-[10px] text-success block">
                    +
                    {(
                      activeMajor.benchmark_2024 - activeMajor.benchmark_2023
                    ).toFixed(2)}{" "}
                    điểm
                  </span>
                </div>
              ...
              <div className="p-3.5 rounded-surface bg-muted/40 border border-border space-y-2 text-xs">
                {activeMajor.tuition_per_credit_vnd === 0 ? (
                  <div className="p-2 rounded-control bg-success/15 text-success font-semibold text-xs">
                    Miễn 100% học phí và nhận trợ cấp sinh hoạt phí hàng tháng
                    theo Nghị định 116 của Chính phủ.
                  </div>
                ) : (
                  <>
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-mono font-bold text-foreground">
                        {activeMajor.tuition_per_credit_vnd.toLocaleString(
                          "vi-VN",
                        )}{" "}
                        đ/tín chỉ
                      </span>
                    ...
                    <div className="flex items-center justify-between border-t border-border/60 pt-2">
                      ...
                      <span className="font-mono font-bold text-primary text-sm">
                        ~
                        {activeMajor.tuition_per_year_vnd.toLocaleString(
                          "vi-VN",
                        )}{" "}
                        đ/năm
                      </span>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-mono text-muted-foreground mt-0.5">
                    Độ trễ phản hồi: {apiResponse.latency_ms}ms • Trạng thái:
                    HTTP 200 OK
                  </p>
                </div>
              </div>
