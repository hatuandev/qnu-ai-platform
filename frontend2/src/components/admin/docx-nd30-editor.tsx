...
import { apiClient, type ToolExecuteResponse } from "../../services/api-client";
...
export const DocxNd30Editor: React.FC<DocxNd30EditorProps> = ({
  ...
  const [formData, setFormData] = useState<DocxNd30FormData>(() => {
    ...
    return {
      ...
      documentNumber:
        initialData.documentNumber || DEFAULT_DOC_DATA.documentNumber,
      ...
      paragraphs: initialData.paragraphs
        ? initialData.paragraphs.map((text, idx) => ({
            id: `p_init_${idx}_${Date.now()}`,
            text,
          }))
        : DEFAULT_PARAGRAPHS,
  ...
  const [lastResponse, setLastResponse] = useState<ToolExecuteResponse | null>(
    null,
  );
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">
              Quy chuẩn lề chuẩn: Trái 30mm, Phải 15mm, Trên 20mm, Dưới 20mm •
          ...
          <div className="p-4 rounded-surface bg-card border border-border space-y-3">
            <div className="flex items-center justify-between">
              <span className="font-semibold text-xs text-foreground flex items-center gap-1.5">
                ...
                <span>
                  Nội Dung Các Đoạn Văn ({formData.paragraphs.length})
                </span>
              </span>
              <Button
                variant="outline"
            ...
            <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
              {formData.paragraphs.map((p, pIdx) => (
                <div
                  key={p.id}
                  className="relative group flex items-start gap-1.5"
                >
              ...
              <div>
                <div className="grid grid-cols-12 gap-2 text-center pb-4 border-b border-neutral-300">
                  ...
                  <div className="col-span-7 space-y-0.5">
                    ...
                    <p className="text-[10px] italic text-neutral-600 pt-1">
                      {formData.cityDate}
                    </p>
                  </div>
                </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground font-mono mt-0.5">
                    ...
                    {(Number(lastResponse.result.size_bytes) / 1024).toFixed(1)}{" "}
                    KB • Lề 20-20-30-15 mm
                  </p>
                </div>
              </div>
