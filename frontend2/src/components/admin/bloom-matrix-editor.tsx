...
export const BloomMatrixEditor: React.FC<BloomMatrixEditorProps> = ({
  onGeneratedSuccess,
}) => {
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 p-3 rounded-surface bg-muted/40 border border-border">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">
              Tự động tính phân bổ tỷ lệ nhận thức, trọng số điểm và kết xuất
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span>
              {isExporting ? "Đang xuất Excel..." : "Xuất File Excel (.xlsx)"}
            </span>
          </Button>
        </div>
      </div>
      ...
      <div className="p-3.5 rounded-surface bg-card border border-border grid grid-cols-1 sm:grid-cols-4 gap-3 text-xs">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 h-8 px-2.5 rounded-control bg-muted/60 border border-border text-xs font-mono font-bold">
            ...
            <span
              className={
                totals.totalScore === 10 ? "text-success" : "text-amber-500"
              }
            >
                ...
                <tr key={t.id} className="hover:bg-muted/30 transition-colors">
                  ...
                  <td className="p-2.5">
                    <input
                      ...
                      value={t.topicName}
                      onChange={(e) =>
                        handleUpdateField(t.id, "topicName", e.target.value)
                      }
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <span className="font-semibold text-foreground">
                Đã kết xuất thành công tệp bảng tính Excel (.xlsx) chuẩn khảo
                thí!
              </span>
              <p className="text-[11px] text-muted-foreground font-mono mt-0.5">
                {String(lastResponse.result.file_name)} • {courseName} (
                {courseCode}) • 4 Cấp độ Bloom
              </p>
            </div>
          </div>
