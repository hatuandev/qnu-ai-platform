...
export const EvaluationRunDetailSheet: React.FC<
  EvaluationRunDetailSheetProps
> = ({ runId, open, onOpenChange }) => {
  ...
  const { data: runDetail, isLoading } = useQuery({
    ...
    queryFn: () =>
      runId
        ...
        : Promise.reject(new Error("runId is required")),
    enabled: !!runId && open,
  });

  const filteredItems = (runDetail?.items || []).filter(
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="right"
        className="w-full sm:max-w-2xl overflow-y-auto p-6 space-y-5"
      >
          ...
          <SheetDescription className="text-xs text-muted-foreground flex flex-wrap items-center gap-3">
            ...
            <span className="flex items-center gap-1">
              ...
              {runDetail?.created_at
                ? new Date(runDetail.created_at).toLocaleString("vi-VN")
                : ""}
            </span>
            <span>•</span>
          ...
          <div className="space-y-5">
            ...
            <div className="flex items-center justify-between text-xs pt-1">
              <span className="font-semibold text-foreground">
                Danh Sách Câu Hỏi Kiểm Định ({filteredItems.length} /{" "}
                {runDetail.total_cases} câu)
              </span>
            ...
            {filteredItems.length === 0 ? (
              ...
              <div className="space-y-3">
                {filteredItems.map(
                  (item: EvaluationResultItem, idx: number) => (
                    <Card
                      key={item.id}
                        ...
                        <div className="space-y-1 bg-muted/40 p-2.5 rounded-md text-[11px]">
                          ...
                          {item.ground_truth && (
                            <div className="text-muted-foreground pl-5 text-[11px] leading-relaxed">
                              <strong>Kỳ vọng (Ground Truth):</strong>{" "}
                              {item.ground_truth}
                            </div>
                          )}
                        </div>
