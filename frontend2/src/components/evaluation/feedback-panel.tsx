    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <KpiMetric
          ...
          value={stats ? `${upRate}%` : "—"}
          helper={
            stats ? `${stats.up}/${stats.total} votes` : "Chưa có vote nào"
          }
          icon={ThumbsUp}
        />
        <KpiMetric
        ...
        <CardContent className="p-4 space-y-2">
          ...
          {points.length === 0 ? (
            <p className="text-xs text-muted-foreground">
              Chưa có dữ liệu vote. Khuyến khích người dùng bấm 👍/👎 dưới mỗi
              câu trả lời.
            </p>
          ) : (
                ...
                <div key={p.date} className="flex items-center gap-2 text-xs">
                  ...
                  <div className="flex-1 h-4 rounded-micro bg-muted overflow-hidden flex">
                    <div
                      ...
                      style={{
                        width: `${((p.up / maxCount) * 100).toFixed(1)}%`,
                      }}
                    />
                    <div
                      className="h-full bg-destructive"
                      style={{
                        width: `${((p.down / maxCount) * 100).toFixed(1)}%`,
                      }}
                    />
                  </div>
