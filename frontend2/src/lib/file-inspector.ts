...
export function inspectFileAndRecommend(
  ...
  if (ext === ".txt" || ext === ".md") {
    return {
      ...
      reason:
        "Tệp văn bản thuần túy không có lớp hình ảnh hay bảng tính phức tạp.",
      technicalDetails:
  ...
  return {
    ...
    technicalDetails:
      "Áp dụng pipeline phân tích tổng quát bảo toàn văn bản và bảng biểu.",
  };
}
...
export function detectDocumentTypeFromFilename(
  ...
  const patterns: [RegExp, string][] = [
    ...
    [
      /(?:de\s*cuong|ma\s*tran|de\s*thi|cau\s*hoi|khao\s*thi)/,
      "de_cuong_mon_hoc",
    ],
    // Giáo trình
  ...
  if (collectionContext) {
    const colId =
      typeof collectionContext === "string"
        ? collectionContext
        : collectionContext.id || "";
    const colName =
      typeof collectionContext === "object" ? collectionContext.name || "" : "";
    const colCode =
      typeof collectionContext === "object" ? collectionContext.code || "" : "";
