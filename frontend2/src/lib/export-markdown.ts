...
export function formatConversationToMarkdown(
  ...
  const { assistantName, assistantCode, assistantDescription, messages } =
    options;
  const now = new Date();
  ...
  for (const msg of messages) {
    ...
    if (msg.role === "user") {
      lines.push(
        `### [Lượt ${turnNumber}] 👤 Người dùng ${timeLabel ? `(${timeLabel})` : ""}`,
      );
      lines.push("");
      ...
      if (msg.citations && msg.citations.length > 0) {
        lines.push(
          `**Căn cứ minh chứng (${msg.citations.length} nguồn tài liệu):**`,
        );
