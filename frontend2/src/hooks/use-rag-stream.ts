    ...
    async (prompt: string, attachments?: ChatAttachment[]) => {
      ...
      try {
        const response = await fetch(
          `/platform/v1alpha1/assistants/${assistantCode}/chat`,
          {
            method: "POST",
            headers: {
            ...
            onEvent: (event: EventSourceMessage) => {
              if (event.event === "token" || !event.event) {
                ...
                try {
                  ...
                  finalCitations.push({
                    ...
                    title:
                      ...
                      "Văn bản trích dẫn",
                    document_name:
                      citationObj.document_name || "Tai_lieu_chinh_thuc.pdf",
                ...
                try {
                  ...
                  if (
                    doneData.suggested_questions &&
                    Array.isArray(doneData.suggested_questions)
                  ) {
                      ...
                      prev.map((msg) =>
                        msg.id === assistantMessageId
                          ? {
                              ...
                              status: "paused_for_approval",
                              approvalId:
                                doneData.approval_id || msg.approvalId,
                            }
                          : msg,
                      ),
                    );
          ...
          const citations: ChatCitation[] = (data.citations || []).map(
            (c: Record<string, unknown>, idx: number) => ({
              ...
              title:
                ...
                "Văn bản trích dẫn",
              document_name:
                (c.document_name as string) || "Tai_lieu_chinh_thuc.pdf",
        ...
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === assistantMessageId ? errorMsgItem : msg,
          ),
        );
