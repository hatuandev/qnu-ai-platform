...
export const evaluationApi = {
  async getEvaluationMetrics(): Promise<EvaluationMetrics> {
    ...
    if (!res.ok) {
      throw new Error(
        `Không tải được chỉ số kiểm định TM-08 (HTTP ${res.status}).`,
      );
    }
  ...
  async getGapInbox(options?: {
    ...
    const res = await fetch(
      `${BASE_URL}/evaluation/gap-inbox${query ? `?${query}` : ""}`,
    );
    if (!res.ok) {
      throw new Error(
        `Không tải được danh sách gap inbox (HTTP ${res.status}).`,
      );
    }
  ...
  async resolveGap(
    ...
    const res = await fetch(
      `${BASE_URL}/evaluation/gap-inbox/${encodeURIComponent(gapId)}`,
      {
        method: "PATCH",
    ...
    if (!res.ok) {
      throw new Error(
        `Không thể đánh dấu xử lý lỗ hổng tri thức (HTTP ${res.status}).`,
      );
    }
    return await res.json();
  },
  ...
  > {
    ...
    if (!res.ok) {
      throw new Error(
        `Không tải được danh sách tập dữ liệu kiểm định (HTTP ${res.status}).`,
      );
    }
    const data = await res.json();
  ...
  async getRunDetail(
    runId: string,
  ): Promise<import("@/types/evaluation").EvaluationRunDetail> {
    const res = await fetch(
      `${BASE_URL}/evaluation/runs/${encodeURIComponent(runId)}`,
    );
    if (!res.ok) {
      throw new Error(
        `Không tải được chi tiết phiên kiểm định (HTTP ${res.status}).`,
      );
    }
    return await res.json();
  },
  ...
  async getRunItems(
    runId: string,
  ): Promise<import("@/types/evaluation").EvaluationResultItem[]> {
    const res = await fetch(
      `${BASE_URL}/evaluation/runs/${encodeURIComponent(runId)}/items`,
    );
    if (!res.ok) {
      throw new Error(
        `Không tải được danh sách câu hỏi kiểm định (HTTP ${res.status}).`,
      );
    }
    const data = await res.json();
    return Array.isArray(data) ? data : [];
  ...
  async runEvaluation(
    ...
    if (!res.ok) {
      throw new Error(
        `Không thể chạy phiên kiểm định TM-08 (HTTP ${res.status}).`,
      );
    }
    return await res.json();
  },
