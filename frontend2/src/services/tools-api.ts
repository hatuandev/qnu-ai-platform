...
export const toolsApi = {
  async getTools(): Promise<ToolItem[]> {
    ...
    return data.map((item, idx) => {
      ...
      const rawName =
        (d.display_name as string) || (d.name as string) || "Tool";
      ...
      return {
        ...
        requires_approval:
          typeof d.requires_approval === "boolean"
            ? d.requires_approval
            : false,
        status: ((d.status as string) ||
          (d.is_active ? "ready" : "maintenance")) as "ready" | "maintenance",
    ...
    try {
      ...
      const errorMsg =
        err instanceof Error
          ? err.message
          : "Mất kết nối tới dịch vụ Tool Gateway máy chủ.";
      return {
        tool_name: payload.tool_name,
        status: "failed",
        error_message: errorMsg,
