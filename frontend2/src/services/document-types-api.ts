...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      problem?.detail ||
        problem?.title ||
        `API loại văn bản lỗi HTTP ${response.status}.`,
    );
  }
...
export function deactivateDocumentType(
  code: string,
): Promise<DocumentTypeItem> {
  return request<DocumentTypeItem>(`/${encodeURIComponent(code)}/deactivate`, {
    method: "POST",
  });
}

export function activateDocumentType(code: string): Promise<DocumentTypeItem> {
  return request<DocumentTypeItem>(`/${encodeURIComponent(code)}/activate`, {
    method: "POST",
  });
}
