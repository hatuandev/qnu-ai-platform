...
export async function listConversations(params?: {
  ...
  if (params?.assistant_code)
    query.set("assistant_code", params.assistant_code);
  if (params?.status && params.status !== "all")
    query.set("status", params.status);
  if (params?.search?.trim()) query.set("search", params.search.trim());

  const url = query.toString()
    ? `${CONVERSATIONS_URL}?${query.toString()}`
    : CONVERSATIONS_URL;
...
export async function getConversationDetail(
  ...
  return requestJson<ConversationThreadDetail>(
    `${CONVERSATIONS_URL}/${encodeURIComponent(id)}`,
  );
}
...
export async function replyConversation(
  ...
  return requestJson<ConversationMessage>(
    `${CONVERSATIONS_URL}/${encodeURIComponent(id)}/reply`,
    {
      method: "POST",
...
export async function getFeedbackStats(
  ...
  const query = assistantCode
    ? `?assistant_code=${encodeURIComponent(assistantCode)}`
    : "";
  return requestJson<FeedbackStats>(
    `${CONVERSATIONS_URL}/feedback/stats${query}`,
  );
}

export async function getFeedbackTrend(days = 14): Promise<FeedbackTrend> {
  return requestJson<FeedbackTrend>(
    `${CONVERSATIONS_URL}/feedback/trend?days=${days}`,
  );
}
