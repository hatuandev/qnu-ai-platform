...
export type AssistantUpdateInput = Partial<
  Omit<AssistantInput, "code" | "tenant_id">
>;
...
export function listAssistants(options?: {
  ...
  return requestJson<AssistantItem[]>(
    `${ASSISTANTS_URL}${query ? `?${query}` : ""}`,
  );
}

export function getAssistant(reference: string): Promise<AssistantItem> {
  return requestJson<AssistantItem>(
    `${ASSISTANTS_URL}/${encodeURIComponent(reference)}`,
  );
}
...
export function seedDefaultAssistants(): Promise<AssistantSeedResult> {
  return requestJson<AssistantSeedResult>(`${ASSISTANTS_URL}/seed-defaults`, {
    method: "POST",
  });
}

export function exportAssistantBundle(
  reference: string,
): Promise<AssistantBundle> {
  return requestJson<AssistantBundle>(
    `${ASSISTANTS_URL}/${encodeURIComponent(reference)}/export`,
  );
}

export function importAssistantBundle(
...
export function generateAssistantSpec(
  ...
  return requestJson<AssistantGeneratedSpec>(`${ASSISTANTS_URL}/generate`, {
    ...
    body: JSON.stringify({
      idea,
      category_hint:
        categoryHint && categoryHint !== "all" ? categoryHint : undefined,
    }),
  });
}
...
export function getAssistantReadiness(
  reference: string,
): Promise<AssistantReadinessResponse> {
...
export function cloneAssistant(
  ...
  return requestJson<AssistantItem>(
    `${ASSISTANTS_URL}/${encodeURIComponent(reference)}/clone`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export function getAssistantVersions(
  reference: string,
): Promise<AssistantVersionItem[]> {
...
export function forkAssistantWorkflow(
  reference: string,
): Promise<AssistantForkWorkflowResponse> {
