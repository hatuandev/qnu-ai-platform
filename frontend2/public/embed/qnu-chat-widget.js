...
(() => {
  ...
  const scriptTag =
    ...
    document.querySelector('script[src*="qnu-chat-widget"]');
  const ASSISTANT_ID =
    scriptTag?.getAttribute("data-assistant") || "ast_admissions";
  const WIDGET_TITLE =
    scriptTag?.getAttribute("data-title") || "Trợ lý AI ĐH Quy Nhơn";
  const WELCOME_MSG =
  ...
  const API_BASE = (
    scriptTag?.getAttribute("data-api-base") || window.location.origin
  ).replace(/\/+$/, "");
