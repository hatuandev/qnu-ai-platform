...
8. **Tái thiết kế toàn diện trải nghiệm Đa thiết bị & Mobile Responsive (Cross-Device Standard)**:
   ...
   - **Lược bỏ viền bao quanh thanh bộ lọc (Borderless Filter Bar)**:
     - Loại bỏ khung viền bao ngoài `border border-border bg-card p-3 shadow-2xs` trên thanh tìm kiếm và bộ lọc của trang Kho tri thức. Các điều khiển Input và Select tự giữ viền riêng thanh thoát, giúp giao diện thông thoáng, liền mạch và hiện đại hơn.
