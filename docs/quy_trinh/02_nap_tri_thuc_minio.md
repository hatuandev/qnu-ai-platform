...
- **Chuẩn Hóa Cấu Trúc Bảng Biểu & Luồng Đọc Đa Tầng (Table Reconstruction & Semantic Order)**:
  * *Nhận diện Header Hình thái học Zero-Keyword (`_is_hdr` & `_has_semantic_headers`)*: Loại bỏ hoàn toàn việc hardcode danh mục từ khóa tiếng Việt; áp dụng 5 tiêu chuẩn hình thái học (loại trừ sequence key, kiểm tra định danh Cột 0, độ dài $\le 45$ ký tự, tỷ lệ số/ngày tháng $\le 20\%$, chứa chữ cái). Tự động phân biệt sub-header ngắn ($\le 25$ ký tự) với câu văn ngắt dòng trôi từ trang trước sang (có chứa dấu `;` hoặc kết thúc bằng `.`).
